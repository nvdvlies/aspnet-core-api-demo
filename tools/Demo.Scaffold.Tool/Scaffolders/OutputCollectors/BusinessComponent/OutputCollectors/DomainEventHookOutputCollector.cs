    ...
    internal class DomainEventHookOutputCollector : IOutputCollector
    {
        public IEnumerable<IChange> CollectChanges(ScaffolderContext context)
        {
            ...
            changes.Add(new CreateNewClass(
                directory: Path.Combine(context.GetEntityDirectory(entityName), "BusinessComponent", "Hooks"),
...
using Demo.Domain.%ENTITY%.BusinessComponent.Events;
using Demo.Domain.Shared.BusinessComponent;
using Demo.Domain.Shared.Interfaces;
using System.Threading;
using System.Threading.Tasks;

namespace Demo.Domain.%ENTITY%.BusinessComponent.Hooks
{
    internal class %ENTITY%CreatedUpdatedDeletedDomainEventHook : IAfterCreate<%ENTITY%>, IAfterUpdate<%ENTITY%>, IAfterDelete<%ENTITY%>
    {
        ...
        public Task ExecuteAsync(HookType type, IBusinessComponentContext<%ENTITY%> context, CancellationToken cancellationToken)
        {
