...
namespace Demo.Infrastructure.Messages
{
    internal class OutboxMessageSender : IOutboxMessageSender
    {
        ...
        public async Task SendAsync(Guid id, CancellationToken cancellationToken = default)
        {
            ...
            {
                _logger.LogError(ex, "Failed to send message of type '{type}'  (ID: {id})",
                    outboxMessageDomainEntity.Type, outboxMessageDomainEntity.EntityId);
            ...
            if (isSent)
            {
                ...
                {
                    _logger.LogCritical(ex, "Failed to mark sent message of type '{type}' as sent (ID: {id})",
                        outboxMessageDomainEntity.Type, outboxMessageDomainEntity.EntityId);
                }
            }
        }

        private async Task UnLockAsync(IOutboxMessageDomainEntity outboxMessageDomainEntity,
        ...
        private async Task UnlockAndMarkAsSentAsync(IOutboxMessageDomainEntity outboxMessageDomainEntity,
            CancellationToken cancellationToken)
        {
            outboxMessageDomainEntity.Unlock();
